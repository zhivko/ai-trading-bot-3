...
class RecurrentFeatureSaliencyCallback(BaseCallback):
    ...
    Handles LSTM states, Fixes CuDNN errors, and supports Smart Labeling.
    """
    def __init__(
        self,
        check_freq: int = 5000,
    ...
    def compute_saliency(self):
        ...
        """
        # CRITICAL FIX 1: CuDNN LSTMs require training mode to compute gradients (backward pass)
        ...
        try:
            ...
            def forward_func(inputs):
                ...
                # CRITICAL FIX 2: Handle Tuple Return from RecurrentPPO
                results = self.model.policy.get_distribution(inputs, (h_expanded, c_expanded), starts_expanded)
            ...
            # 5. Process Results & SMART LABELING
            attrs = attributions.detach().cpu().numpy()[0]
            n_inputs = len(attrs)
            final_labels = []
            
            # Logic to handle name mismatch gracefully
            if self.feature_names:
                ...
                for i in range(n_inputs):
                    ...
                    if i < n_names:
                        ...
                        final_labels.append(f"{f_id} ({clean_name})")
                        
                    # Scenario B: Windowed Match (e.g. Input 100, Names 50 -> Index 0 of Lag 2)
                    elif n_names > 0 and n_inputs > n_names and n_inputs % n_names == 0:
                        base_idx = i % n_names
                        lag = i // n_names
                        clean_name = self.feature_names[base_idx]
                        final_labels.append(f"{f_id} ({clean_name}_t-{lag})")
                        
                    # Scenario C: Overflow / Unknown
                    else:
                        final_labels.append(f"{f_id} (Unknown)")
            else:
                # No names provided
                final_labels = [f"F_{i}" for i in range(n_inputs)]

            # Create DataFrame
            df_attrs = pd.DataFrame({'Feature': final_labels, 'Importance': attrs})
            ...
            if wandb.run is not None:
                try:
                    wandb.log({"Explainability/Feature_Saliency": wandb.Image(plt.gcf())})

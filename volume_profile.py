...
def generate_cache_filename(df, window_days, num_bins, cache_dir="vp_cache"):
    ...
    param_signature = f"win{window_days}_bins{num_bins}"
...
def compute_volume_profile(df_window, num_bins=40):
    """
    Compute Volume Profile for a given window of OHLCV data.
    """
    if df_window.empty:
        return {'poc': 0, 'vah': 0, 'val': 0, 'heatmap': np.zeros(40)}
    ...
    if min_p == max_p:
        return {'poc': min_p, 'vah': min_p, 'val': min_p, 'heatmap': np.zeros(40)}
...
def compute_vp_for_window(window_df, num_bins):
    """
    Helper function for multiprocessing: compute VP for a single window.
    """
    return compute_volume_profile(window_df, num_bins)

def get_rolling_vp(df, window_days, num_bins=40, cache_dir="vp_cache"):
    """
    ...
    # 2. Check for Cache
    cache_path = generate_cache_filename(df, window_days, num_bins, cache_dir)
    ...
    heatmaps = np.zeros((len(df), 40), dtype=np.float32)
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        func = partial(compute_vp_for_window, num_bins=num_bins)
